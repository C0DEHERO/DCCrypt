...
EncryptDecryptFolder::EncryptDecryptFolder(QObject *parent) :
    EncryptDecryptFile(parent)
{
    stopped = false;
    processSubdirs = true;
    followSymLinks = true;
...
void EncryptDecryptFolder::stopQueue()
{
    stopped = true;
}

void EncryptDecryptFolder::startOperation(const QString &folderPath)
{
    QDir dir(folderPath);
        ...
        if(!processSubdirs && !followSymLinks)
    ...
    if(dir.exists())
    {
        ...
        for(int i = 0; i < list.size(); i++)
        {
            ...
            if(fileInfo.isDir())
            {
                if(fileInfo.exists())
                 {
                    QString outputFolderPathBak(this->outputFolderPath);
                    QString outputPath;
            ...
            else if(fileInfo.isFile())
            {
                if(fileInfo.exists())
                {
                    ...
                    encryptDecrypt(fileInfo.absoluteFilePath(), outputFileName, keyFileName);

                    emit madeProgress(fileInfo.size());
                }
            }
        }
...
void EncryptDecryptFolder::setInputFolderPath(const QString &inputFolderPath)
{
    this->inputFolderPath = inputFolderPath;
}

void EncryptDecryptFolder::setOutputFolderPath(const QString &outputFolderPath)
{
    this->outputFolderPath = outputFolderPath;
}

void EncryptDecryptFolder::setKeyFileName(const QString &keyFileName)
{
    this->keyFileName = keyFileName;
}

void EncryptDecryptFolder::readSettings(QString settingsFileName)
{
